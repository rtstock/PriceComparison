...
:: usage get-latest-evare-file-specific.bat PershingIPCPrice
:: usage get-latest-evare-file-specific.bat ComericaIPCPrice
:: usage get-latest-evare-file-specific.bat WFAIPCPrice

::-----------------------------
:: First Set the main variables
Set AppDir=D:\ClientData\Commands\projects\price_comparison\code
Set DestDir=D:\ClientData\Commands\projects\price_comparison\get-destination
...
echo Today in 8 characters is %datespecific8%
...
call "%AppDir%\ftp_get_file_from_sscgateway_usr_ssc519_evare.bat" "%1%datespecific8%*.csv" "%DestDir%"
...
if exist %DestDir%\%1%datespecific8%*.csv (
    echo success: today's file %1%datespecific8%*.csv was retrieved successfully
    goto success_exit
) else (
    echo fail: Today's file %1%datespecific8%*.csv was not retrieved
    goto fail_ftp
)
...
:: Exit on FTP failure
:fail_ftp
echo Exited for failing to retrieve %1%datespecific8%.csv from FTP site
